...
- **Rate resolution** (`src/lib/server/rates.ts`): rate plans are versioned by `effectiveFrom` date. `resolveRatePlan` picks the plan with the latest `effectiveFrom` that's still `<=` the session date, so historical sessions keep the rate that was actually in effect. For `peak_offpeak` plans, the _entire_ session's kWh is billed at whichever rate applies at the session's start time (an intentional billing approximation, not a bug: usage is never split across a peak/off-peak boundary. Only Evnex-imported sessions carry `started_at`/`ended_at` at all — a manually-logged session has just a start date/time — and even with both instants there's no per-interval meter data to say how much of the kWh fell on each side of the boundary).
...
- **Run `npm run check`, `npm run lint` and `npm run test` before opening a PR.** There's no CI gate on pull requests here (the GitHub Actions workflows only run on pushes to `main` and on `v*` tags), so these local runs are the only check before review. `npm run lint` currently also flags a pre-existing prettier warning on `RELEASE_NOTES.md`; call that out in the PR rather than letting it hide a new one. For any UI change, also do the Playwright pass described under "Browser testing" above.
- **PR descriptions** follow the existing format: a `## Summary` of bullets (what changed and why, not a file list) and a `## Test plan` checklist. Tick only what was actually run, and leave anything not verified (a real third-party account, the Electron build) unticked rather than omitting it.
- Don't force-push, don't skip hooks, and don't merge or deploy without explicit confirmation. This is a solo-maintained app with no staging environment: a push to `main` publishes a Docker image, so anything that touches `main` effectively touches production on the next deploy.
- **Stop any dev server process you started (`npm run dev`, `vite`, `vitest` in watch mode, etc.) once you're done with it**, including any child processes it spawned. The one exception: a dev server that was already running before you started (the user's own persistent session). Leave that alone, and check process start times if unsure whether something predates your session. Browser testing against a scratch database (`DATABASE_URL=<scratch path>`) is preferred to touching the real `data/` DB.
- **PR review loop: CodeRabbit or Kilo Code is the reviewer, and every thread it opens must end resolved before merge — never left open on a merged PR.** After opening a PR, wait for its reviewer check (`CodeRabbit` or `Kilo Code Review`) and any inline comments (`gh pr checks <n>` / `gh api repos/<owner>/<repo>/pulls/<n>/comments`) — don't merge while a reviewer comment sits unaddressed. For each finding: fix it (or give a clear reason if you're not going to), **reply on that exact review thread** stating what changed and why (`gh api repos/<owner>/<repo>/pulls/comments -f body="..." -F in_reply_to=<comment_id>` — note `-F`, not `-f`, for `in_reply_to`: the API rejects it as a string), then resolve the thread (GraphQL `resolveReviewThread`, thread id from `reviewThreads` under the PR). Do this even if you fix the issue in a later commit on the same PR — the original comment stays open until you close the loop on it explicitly; pushing a fix alone does not resolve or reply to anything. If you only realize a thread was left open after merging, still go back and reply + resolve it on the merged PR — don't leave it dangling because the PR is already closed.
- **Cutting a release (`vX.Y.Z`)**: pushing a `v*` tag triggers `docker-publish.yml` and `electron-release.yml` (three-platform desktop builds, published by electron-builder). Write the real notes as soon as the tag is pushed (`gh release create vX.Y.Z --notes-file …`), without waiting for CI: electron-builder otherwise creates the release with an empty body, and it stays that way for the several minutes the builds take. Draft them in `RELEASE_NOTES.md` on a branch beforehand. Iterate on desktop-app fixes with local Electron builds rather than cutting a release per attempt.
