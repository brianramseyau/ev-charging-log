...
[] Enhancement: add date and time pickers in the sessions adding page, a date picker already exists in the periods add page so this can be re-used (ensure its turned into a component if code-reuse is required and not built-in).