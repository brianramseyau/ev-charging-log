...
Status: **app side (Phases 2–7) and companion (Phase 1, in
`brianramseyau/ev-charging-log-hass`) implemented; Phase 0's real-HA capture
pending.** Confirmed: the user already runs the
...
**The companion lives in its own repository, `brianramseyau/ev-charging-log-hass`,
...
Local DNS configuration (making sure the Unraid host and its containers
resolve the hostname through the split-horizon record) is the user's network
setup and **out of scope for this plan**. The app is correct on either path;
it just calls the URL.
...
- **Phase 0 checks the endpoint over the Cloud relay** (§10), from outside the
  LAN (a phone hotspot), since that's the desktop app's away path.
...
| #   | Phase                                                                                                              | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | **Capture a real history by hand** (about 15 minutes, no code)                                                     | Using the user's own browser session or a temporary token that's deleted right after, `curl` HA's built-in `/api/history/period` for the two entities over a recent overnight charge, once through the Cloud URL from outside the LAN (a phone hotspot). This confirms the Cloud path (§4.5), the entity IDs, the telemetry timestamp format, and that the charge window contains several unchanged odometer readings at the user's poll interval. The redacted output becomes the fixture for both repos. The temporary token never goes near the app. |
| 1   | **The companion integration**, in `brianramseyau/ev-charging-log-hass` (not this repo)                             | View, config flow, options flow, tests, HACS metadata. Ends with the user installing it from HACS and `curl`-ing the endpoint with the secret.                                                                                                                                                                                                                                                                                                                                                                                                          |
