...
| 7   | **Every call to HA is made by the app's server, never the browser.** The page only calls this app's own `+server.ts` endpoints (§7.1, §7.2), and the SvelteKit server calls the companion. The secret never reaches the browser, and what decides whether HA is reachable is where the app's **server** runs, not where the user's phone is (§4.5).                                                                                                                                                                                                                                     |
...
- **Away from home:** irrelevant for the Docker/Unraid install. Its server
  sits on the home network, so it always reaches HA locally, whether the phone
  using the app is at home or not (§4.5). For the desktop app, whose server is
  the laptop itself, the same URL reaches HA through Home Assistant Cloud when
  the laptop is away. If HA can't be reached at all (HA down,
...
**The companion lives in its own repository, `brianramseyau/ha-ev-charging-log`,
and nowhere else.** It's a hard boundary, not a convenience:

- **Nothing of the companion lives in this repo.** No Python, no
  `custom_components/` folder, no git submodule, no build step, no copy of its
  code "for reference". HACS installs from a repository's root layout, so a
  subfolder of this app couldn't be installed from HACS anyway.
- **The only coupling is the HTTP contract in §4.3.** It's versioned by the
  response's `version` field and documented in the companion's README. This
  repo only ever calls the endpoint. A breaking change to the contract bumps
  `version` in the companion, and the app shows "update the companion" (§7.1)
  instead of misreading the response.
- **Each repo has its own releases, CI and issues.** The companion is released
  through GitHub releases, which HACS tracks, and has its own changelog and
  hassfest/HACS validation workflows. The app's releases don't depend on it.
- **Fixtures are copied, not shared.** The redacted Phase 0 capture goes into
  each repo's own test data. There's no cross-repo import.

The repo is laid out like `hass-byd-vehicle`: `custom_components/ev_charging_log/`
at the root, `hacs.json`, `README.md`, `LICENSE` (MIT), and the validation
workflows.
...
`https://` URL. None of this is special-cased in code: the app just calls a
URL.

Because every call is made server-side (§2 #7), **the network path depends on
where the app's server runs, not where the user is:**

| Deployment                     | Where the server runs    | Path to HA                                                                                                                                                                                         |
| ------------------------------ | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Docker on Unraid** (primary) | The home network, always | **Always local**, through split DNS. A phone using the app from anywhere doesn't change this: the phone talks to the app, and the app talks to HA over the LAN. The Cloud relay is never involved. |
| **Electron desktop app**       | The laptop               | Local at home (split DNS); through the Cloud relay when the laptop is away.                                                                                                                        |

One thing to check for the Docker case: the container has to resolve the
hostname through the home DNS server that holds the split record. Docker
containers inherit the host's resolver by default, so if Unraid uses the home
DNS, this just works. If Unraid points at a public resolver (1.1.1.1, say),
the name resolves to Nabu Casa's relay instead. That still works, just through
the relay, and Phase 0 checks which path is used.
...
- **The relay adds latency.** The timeout is 10 s (§6), not the 5 s you'd
  pick for a LAN-only call. That only matters for the desktop app away from
  home, but one timeout keeps it simple. Recorder queries for a few days of two entities
  are small, so there's plenty of headroom.
- **If the Cloud subscription lapses,** the Docker install is unaffected
  (it's always local). Only the desktop app away from home breaks, and it just
  reports "unreachable" until it's back, with no data loss (§3).
- **Phase 0 checks both paths** (§10): from inside the Docker container on
  Unraid (`docker exec … curl`, which confirms split DNS resolves to the local
  instance), and over the Cloud relay from outside the LAN (a phone hotspot,
  the desktop app's away path).
...
| #   | Phase                                                                                                              | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --- | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | **Capture a real history by hand** (about 15 minutes, no code)                                                     | Using the user's own browser session or a temporary token that's deleted right after, `curl` HA's built-in `/api/history/period` for the two entities over a recent overnight charge, once from inside the app's Docker container on Unraid and once through the Cloud URL from outside the LAN (a phone hotspot). This confirms both network paths (§4.5), the entity IDs, the telemetry timestamp format, and that the charge window contains several unchanged odometer readings at the user's poll interval. The redacted output becomes the fixture for both repos. The temporary token never goes near the app. |
| 1   | **The companion integration**, in `brianramseyau/ha-ev-charging-log` (not this repo)                               | View, config flow, options flow, tests, HACS metadata. Ends with the user installing it from HACS and `curl`-ing the endpoint with the secret.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
